...
const Dashboard = () => {
  ...
  // ✅ Only show overall % after ALL classes have marked attendance (i.e., every class has total > 0 today)
  const allClassesMarkedToday = perClass.length > 0 && perClass.every((c) => c.total > 0);
  const studentPercent = allClassesMarkedToday && todayTotal > 0
