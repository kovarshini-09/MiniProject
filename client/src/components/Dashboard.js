...
const Dashboard = () => {
  ...
  // ✅ You can connect these to backend later easily
  const totalStudents = 60;
  const totalTeachers = 19;
  const presentStudents = 42; // Example data — replace dynamically later
  const presentTeachers = 15; // Example data

  // ✅ Calculate percentage dynamically
  const studentPercent = Math.round((presentStudents / totalStudents) * 100);
  const teacherPercent = Math.round((presentTeachers / totalTeachers) * 100);
    ...
    <Container fluid className="p-4 bg-light min-vh-100">
      ...
      <Row className="g-4">
        ...
        <Col lg={8}>
          ...
          <div className="d-flex flex-column gap-3">
            <Card className="border rounded shadow-sm p-3">
              <p className="fw-semibold mb-1">Today Present Students</p>
              <p className="text-secondary mb-0">Attendance not marked yet !</p>
            </Card>
